...
## 3. VPS / VDS — рекомендуется для продакшена

Постоянная PostgreSQL, картинки, CI/CD из GitHub.

**Пошаговая инструкция:** **[VPS.md](./VPS.md)**

| Файл | Назначение |
|------|------------|
| `docker-compose.vps.yml` | полный стек на сервере |
| `.github/workflows/deploy-vps.yml` | автодеплой при push |
| `deploy/vps.env.example` | переменные на сервере |

---

## 4. API на VPS (старый вариант — только API)
...
## 5. Альтернативы (РФ, оплата в рублях)
